...
export class EditAgencyComponent implements OnInit {
  ...
  constructor(private service: CommonService, private router: Router, private activatedRouter: ActivatedRoute,
    ...
    this.service.setTitle('Edit Agency | Nika Trading');
  ...
  public async get_data() {
    ...
    this.result = await this.service.get_action(formData, '/agencies/get-data/' + this.id);
  ...
  public async doFormAction() {
    ...
    this.loader.show();
    this.result = await this.service.update_action(this.form.value, '/agencies/edit/' + this.id);
    if (this.result.status == true) {
      this.loader.hide();
      setTimeout(() => {
