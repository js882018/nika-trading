...
export class EditOrderComponent implements OnInit {
  ...
  constructor(private service: CommonService, private router: Router, private activatedRouter: ActivatedRoute,
    ...
    this.service.setTitle('Edit Order | Nika Trading');
    ...
    if (this.result == 'unknown_error') {
      ...
      if (this.result.status == true) {
        ...
        this.form = this.fb.group({
          ...
          total_amount: [this.data.total_amount, [Validators.required, Validators.pattern('\^([\\d]{0,10})(\\.|$)([\\d]{1,4}|)$')]],
          shipment_date: [this.data.shipment_date],
          user_id: this.login.getSessionData('session_id'),
          order_items: this.fb.array([])
        });
        this.result.item_details.forEach((element: any, index: any) => {
          this.order_items.push(this.fb.group({
            id: [element.id],
            item_id: [element.item_id, this.checkDuplicateItem()],
            item_name: [element.item_name, Validators.required],
            quantity: [element.quantity, [Validators.required, Validators.pattern('^[0-9]*$')]],
            unit_price: [element.unit_price, [Validators.required, Validators.pattern('\^([\\d]{0,10})(\\.|$)([\\d]{1,4}|)$')]],
  ...
  public async doFormAction() {
    ...
    this.loader.show();
    this.result = await this.service.update_action(this.form.value, '/orders/edit/' + this.id);
    if (this.result.status == true) {
      this.loader.hide();
      setTimeout(() => {
